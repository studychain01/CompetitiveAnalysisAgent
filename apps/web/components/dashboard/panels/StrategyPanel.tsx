...
function horizonBadge(h: string) {
  const low = h.toLowerCase();
  const isShort = low.includes("short") || low.includes("0–90") || low.includes("0-90");
  return (
    <span
      className={
        isShort
          ? "inline-flex rounded-full bg-accent-subtle px-2 py-0.5 text-xs font-semibold text-accent"
          : "inline-flex rounded-full border border-border bg-surface-elevated px-2 py-0.5 text-xs font-medium text-muted"
      }
    >
      {h || "—"}
    </span>
...
function effortBadge(e: string) {
  ...
  const heavy = low.includes("heavy");
  return (
    <span
      className={
        heavy
          ? "inline-flex rounded-full bg-warning/15 px-2 py-0.5 text-xs font-medium text-warning"
          : "inline-flex rounded-full border border-border px-2 py-0.5 text-xs font-medium text-muted"
      }
    >
      {e || "—"}
    </span>
      ...
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
            <h3 className="text-[17px] font-semibold leading-snug tracking-tight text-fg">{title || "Move"}</h3>
          ...
          {rationale ? (
            <p className="mt-2.5 max-w-prose text-pretty text-[15px] leading-relaxed text-muted">{rationale}</p>
          ) : null}
          {risk ? (
            <div className="mt-3 rounded-lg border border-danger/15 bg-danger/[0.04] px-3 py-2.5">
      ...
      <table className="w-full min-w-[560px] border-collapse text-left text-sm">
        <thead>
          <tr className="border-b border-border bg-surface-elevated text-xs uppercase tracking-wide text-subtle">
            ...
            <th className="p-3 font-semibold">Horizon</th>
        ...
        <tbody>
          {moves.map((row, i) => (
            <tr key={i} className="border-b border-border last:border-0">
              ...
              <td className="p-3 align-top">{horizonBadge(String(row.horizon ?? ""))}</td>
              <td className="p-3 align-top">{effortBadge(String(row.effort ?? ""))}</td>
    ...
    <div className="mx-auto max-w-5xl space-y-10 pb-8">
      ...
      <section className="space-y-6">
        <StrategySectionIntro
          chip="Start here"
          title="Top moves across competitors"
          description="Ranked actions that apply across your competitive set. Horizon and effort tags set scope; each move includes a short risk line for your company when the model provides one."
          accent="indigo"
        />
        {moves.length === 0 ? (
